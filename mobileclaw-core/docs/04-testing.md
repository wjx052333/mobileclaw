...
| `memory_doc_created_at_is_set` | `src/memory/types.rs` | unit | `MemoryDoc::new` generates a non-empty `id`, preserves `path`, and stores the correct `category` |
| `search_result_ordering` | `src/memory/types.rs` | unit | A `Vec<SearchResult>` sorted by descending score places the higher-scored result first |
...
**`src/memory/types.rs`** — Core memory data types. Covers `MemoryDoc::new` (non-empty ID generation, path and category preservation) and `SearchResult` score-based ordering.
