...
- [x] Add `encryption_key: Vec<u8>` to `AgentConfig` in `ffi.rs` (2026-04-01)
- [x] Replace hardcoded AES dev key with `config.encryption_key` (2026-04-01)
- [x] Remove `compile_error!` guard blocking release builds (2026-04-01)
- [x] Update `frb_generated.rs` codecs (SseDecode/SseEncode/IntoDart) to handle 8-field `AgentConfig` (2026-04-01)
- [ ] Replace hardcoded AES dev key with a key derived from `flutter_secure_storage` (cross-platform):
  ...
  - Dart: pass the 32 raw bytes when calling `AgentSession.create`
...
- [x] Install `cargo-ndk` and verify `arm64-v8a` build succeeds (2026-04-01, rebuilt to include email FFI)
- [x] Verify `x86_64` build succeeds (2026-04-01, for emulator testing)
- [x] Confirm `.so` files are loaded at runtime via Gradle `jniLibs` (2026-04-01, app runs on emulator-5554)
